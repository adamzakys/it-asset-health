...
function openForm(item) {
  ...
  // Simpan ke riwayat (Pakai Try-Catch biar kalau error gak bikin macet)
  try {
    saveToRecentScans(item);
  } catch (e) {
    console.warn("Gagal simpan riwayat:", e);
  }
  ...
  // 2. Reset Form (Panggil fungsi yg baru kita perbaiki di atas)
  resetFormUI();

  // 3. Atur Tampilan (Switch View)
  const listView = document.getElementById("patrol-list-view");
  const recentView = document.getElementById("recentScanContainer");
  const formView = document.getElementById("patrol-form-view"); // Pastikan ID ini benar di HTML

  if (listView) listView.classList.add("hidden");
  if (recentView) recentView.classList.add("hidden");

  // MUNCULKAN FORM
  if (formView) {
    formView.classList.remove("hidden");
  } else {
    alert("Error: ID 'patrol-form-view' tidak ditemukan di HTML!");
    return;
  }

  // 4. Isi Teks Data Aset
  const elName = document.getElementById("formAssetName");
  const elLoc = document.getElementById("formAssetLoc");
  const elId = document.getElementById("formAssetId");

  if (elName) elName.innerText = item.name || "Nama Tidak Ada";
  if (elLoc) elLoc.innerText = item.location || "-";
...
function resetFormUI() {
  ...
  // Reset Status ke Normal
  const statusBtn = document.querySelector(".status-btn");
  if (statusBtn) setStatus("Normal", statusBtn);

  // --- PERBAIKAN DI SINI (Sesuaikan dengan ID Baru) ---

  // 1. Reset Input Kamera
  const camInput = document.getElementById("file_cam_lapor");
  if (camInput) camInput.value = "";

  // 2. Reset Input Galeri
  const galInput = document.getElementById("file_gal_lapor");
  if (galInput) galInput.value = "";

  // 3. Reset Preview & Placeholder
  const prevImg = document.getElementById("prev_img1");
  if (prevImg) {
    prevImg.src = "";
    prevImg.classList.add("hidden");
  }

  const placeholder = document.getElementById("placeholder_img1");
  if (placeholder) placeholder.classList.remove("hidden");
  // ----------------------------------------------------

  // Reset History List
  const histList = document.getElementById("assetHistoryList");
  if (histList) {
    histList.innerHTML = "";
    histList.classList.add("hidden");
  }
  const histIcon = document.getElementById("histIcon");
  if (histIcon) histIcon.innerText = "expand_more";
}
...
async function submitLaporan() {
  ...
  try {
    const elCam = document.getElementById("file_cam_lapor");
    const elGal = document.getElementById("file_gal_lapor");
    const fileCam = elCam ? elCam.files[0] : null;
    const fileGal = elGal ? elGal.files[0] : null;
    const finalFile = fileCam || fileGal;
    ...
    const payload = {
      ...
      catatan: catatan,
      petugas: currentUser,
      fotoKondisi: finalFile ? await toBase64(finalFile) : "",
    };

    await fetch(API_URL, { method: "POST", body: JSON.stringify(payload) });

    // === UPDATE DATA SHARE WA LAPORAN ===
    tempShareData = {
      type: "laporan", // Penanda tipe
    ...
    // Kembalikan judul modal sukses (jika tadi diubah jurnal)
    const successTitle = document.querySelector("#modalSuccess h3");
    if (successTitle) successTitle.innerText = "Laporan Terkirim!";

    const btnWA = document.getElementById("btnShareWA");
    if (btnWA) btnWA.onclick = executeShareWA;
  } catch (e) {
    closeModal("modalLoading");
...
function scanFromFile(input) {
  if (input.files && input.files.length > 0) {
    ...
    document.getElementById("loadingText").innerText = "Memindai Gambar...";
    openModal("modalLoading");

    // Gunakan Html5Qrcode library
    const html5QrCode = new Html5Qrcode("reader");
    html5QrCode
      .scanFile(imageFile, true)
      .then((decodedText) => {
        closeModal("modalLoading");
        console.log("Scan File Sukses:", decodedText);
        processScanResult(decodedText);
      })
      .catch((err) => {
        closeModal("modalLoading");
        alert("QR Code tidak terbaca di gambar ini.");
      });
  }
}
...
async function submitJurnal() {
  const judulEl = document.getElementById("jurnal_judul");
  const lokasiEl = document.getElementById("jurnal_lokasi");
  const descEl = document.getElementById("jurnal_desc");

  if (!judulEl || !lokasiEl || !descEl) {
    alert("Error elemen input.");
    return;
  }
  ...
  try {
    // Ambil File (Kamera/Galeri)
    const elCam = document.getElementById("file_cam_jurnal");
    const elGal = document.getElementById("file_gal_jurnal");
    const fileCam = elCam ? elCam.files[0] : null;
    const fileGal = elGal ? elGal.files[0] : null;
    const finalFile = fileCam || fileGal;

    const base64Foto = finalFile ? await toBase64(finalFile) : "";
    const payload = { type: "jurnal", judul, lokasi, deskripsi: desc, foto: base64Foto, petugas: currentUser };

    await fetch(API_URL, { method: "POST", body: JSON.stringify(payload) });

    // === SIAPKAN DATA SHARE WA JURNAL ===
    tempShareData = {
      type: "jurnal", // Penanda tipe
      judul: judul,
      lokasi: lokasi,
      desc: desc,
      img1: finalFile, // File gambar asli untuk di-share
    };
    ...
    // === TAMPILKAN MODAL SUKSES (Biar bisa Share WA) ===
    openModal("modalSuccess");

    // Ubah teks di modal sukses sedikit biar nyambung
    const successTitle = document.querySelector("#modalSuccess h3");
    if (successTitle) successTitle.innerText = "Jurnal Tersimpan!";
    ...
    descEl.value = "";
    lokasiEl.value = "";
    const prev = document.getElementById("prev_img_jurnal");
    if (prev) {
      prev.src = "";
      prev.classList.add("hidden");
    }
    const place = document.getElementById("placeholder_jurnal");
...
// === HELPER WAJIB (JANGAN SAMPAI TIDAK ADA) ===
...
function triggerFile(id) {
  ...
  if (el) {
    ...
    console.error("Input ID tidak ditemukan:", id);
  }
}

function previewImg(input, imgId, otherInputId) {
  if (input.files && input.files[0]) {
    const reader = new FileReader();
    reader.onload = function (e) {
      ...
      const placeholderId = "placeholder_" + imgId.replace("prev_", "");
