...
let targetInputId = null; // Untuk mode scan-to-input
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Set default date hari ini di modal add
...
function switchTab(tabName, btn) {
  ...
  setTimeout(() => {
    ...
    // Sedikit delay biar transisi display block ke opacity jalan
    setTimeout(() => target.classList.add("active"), 10);
  }, 100); // Durasi transisi fade out
...
async function fetchData(action, query) {
  ...
  try {
    ...
    if (data.length === 0) {
      // LOGIKA SMART SCAN: Jika scan & tidak ketemu -> Tawarkan Tambah Baru
      if (action === "scan") {
        if (confirm(`ID Aset: ${query}\nData tidak ditemukan di Master.\n\nTambahkan sebagai Aset Baru?`)) {
          openModal("modalAddAsset");
          document.getElementById("new_id").value = query; // Auto fill ID
        }
      } else {
...
function renderSearchResults(data) {
  ...
  container.innerHTML = `<div class="text-xs text-slate-500 font-bold px-1 mb-2">Ditemukan ${data.length} aset:</div>`;
  data.forEach((item) => {
    const div = document.createElement("div");
...
function startScanner() {
  targetInputId = null; // Reset mode input
  openModal("modalScanner");
  startHtml5Scanner();
}

function scanForInput(inputId) {
  targetInputId = inputId; // Set mode input
  openModal("modalScanner");
  startHtml5Scanner();
}
...
function startHtml5Scanner() {
  ...
  html5QrcodeScanner
    ...
    .catch((err) => {
      alert("Kamera Error/Permission Denied.");
...
// ================= FORM & TOOLS (CRUD) =================
// (Bagian ini sama, hanya styling active:scale diperbaiki di HTML)
function renderTools() {
  ...
  myTools.forEach((tool) => {
    ...
    if (tool.qr && tool.qr.startsWith("http")) {
      qrSection = `<div class="mt-3 flex justify-center"><img src="${tool.qr}" class="w-24 h-24 border p-1 rounded shadow-sm"></div>`;
    }
    div.innerHTML = `
            ${btnEdit}
            <div class="flex gap-3">
                <div class="bg-blue-50 p-2.5 rounded-xl text-accent h-fit shadow-sm border border-blue-100">
                    <span class="material-icons-round">${tool.icon || "build"}</span>
                </div>
                <div class="flex-1 overflow-hidden">
                    <h3 class="font-bold text-slate-700 text-sm">${tool.name}</h3>
                    ...
                    <button onclick="copyToClipboard('${tool.cmd.replace(
                      /'/g,
                      "\\'"
                    )}')" class="text-[10px] font-bold text-accent bg-blue-50 border border-blue-100 px-3 py-1.5 rounded-lg active:scale-90 transition-transform">Copy Script</button>
                    ${qrSection}
                </div>
            </div>
        `;
...
function saveTool() {
  ...
  if (id) {
    ...
    if (index !== -1) myTools[index] = { id: parseInt(id), name, desc, icon, cmd, qr };
  } else {
...
async function submitLaporan() {
  ...
  try {
    ...
    // 🚀 RESTORE FITUR WA: Langsung buka WA setelah sukses
    await shareToWA(currentAsset.name, status, catatan, img1, img2);
...
async function shareToWA(nama, status, note, f1, f2) {
  const caption = `*LAPORAN ASET*\n` + `📍 Alat: ${nama}\n` + `🔧 Status: ${status.toUpperCase()}\n` + `📝 Catatan: ${note || "-"}\n` + `👮 Petugas: ${currentUser}`;

  const filesArray = [];
  if (f1) filesArray.push(new File([f1], "kondisi.jpg", { type: f1.type }));
  if (f2) filesArray.push(new File([f2], "label.jpg", { type: f2.type }));
